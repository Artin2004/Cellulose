...
async function checkUrl(plant, id) {
  const url = `https://images.unsplash.com/${id}?w=600&h=400&fit=crop`;
  return new Promise((resolve) => {
    https.get(url, (res) => {
      res.resume();
      if (res.statusCode !== 200) {
        console.log(`BROKEN (${res.statusCode}): ${plant}`);
      } else {
        console.log(`OK: ${plant}`);
      }
      resolve();
    }).on('error', (e) => {
      console.log(`ERROR: ${plant} - ${e.message}`);
      resolve();
    });
  });
}
...
(async () => {
  const entries = Object.entries(unsplashIds);
  for (const [plant, id] of entries) {
    await checkUrl(plant, id);
